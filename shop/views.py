...
def product(request , id_prod ):
    ...
    #imag = Product_img.objects.get(id = id_img)
...
def authorizd(request):
    user = None
    info = [ {"У ВАС НЕТ АККАУНТА, ПОЖАЛУЙСТА ЗАРЕГИСТРИРУЙТЕСЬ"} , {"Вы успешно зарегистрировались. Приветствуем Вас на нашем сайте."} ]
    if request.method == "POST":
        data = request.POST
        user = User.objects.create_user(username = data["username"] , email = data["email"] , password = data["password"])
        user.save()
